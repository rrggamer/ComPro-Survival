...
void Game::innitText()
{
	...
	this->uiText.setPosition(200.f, 200.f);
...
void Game::initTexture()
{
	...
	enemyTextures["ENEMY_TYPE_1"] = new sf::Texture();
	enemyTextures["ENEMY_TYPE_1"]->loadFromFile("Assets/Monster/enemy_type_1.png");
	enemyTextures["ENEMY_TYPE_2"] = new sf::Texture();
	enemyTextures["ENEMY_TYPE_2"]->loadFromFile("Assets/Monster/enemy_type_2.png");
	enemyTextures["ENEMY_TYPE_3"] = new sf::Texture();
	enemyTextures["ENEMY_TYPE_3"]->loadFromFile("Assets/Monster/enemy_type_3.png");
	enemyTextures["ENEMY_TYPE_4"] = new sf::Texture();
	enemyTextures["ENEMY_TYPE_4"]->loadFromFile("Assets/Monster/enemy_type_4.png");
	enemyTextures["ENEMY_TYPE_5"] = new sf::Texture();
	enemyTextures["ENEMY_TYPE_5"]->loadFromFile("Assets/Monster/enemy_type_5.png");
...
Game::Game()
{
	...
	isFirstMaxHPIncrease = true;
	isFirstMaxHPIncrease = true;
	isSecondMaxHPIncrease = true;
	isThirdMaxHPIncrease = true;
	isForthMaxHPIncrease = true;
	isFirstTextureChange = true;
	isSecondTextureChange = true;
	this->initVariables();
	this->initWindow();
	this->initEnemies();
	this->initPlayer();
...
void Game::updateInput()
{
	if (this->canAttack())
	{
		...
		for (size_t i = 0; i < std::min(distances.size(), size_t(this->target)); ++i)
		{
...
void Game::spawnEnemy()
{
	...
	*/
	...
	case 0:
		...
		this->enemy.setTexture(enemyTextures["ENEMY_TYPE_1"]);
	...
	case 1:
		...
		this->enemy.setTexture(enemyTextures["ENEMY_TYPE_2"]);
		enemy.setTextureRect(sf::IntRect(0, 0, 64, 64));
		this->enemyHp = 40;
		break;
	case 2:
		this->enemy.setSize(sf::Vector2f(200.f, 200.f));
		this->enemy.setFillColor(sf::Color::Cyan);
		this->enemy.setTexture(enemyTextures["ENEMY_TYPE_3"]);
		enemy.setTextureRect(sf::IntRect(0, 0, 64, 64));
		this->enemyHp = 60;
		break;
	case 3:
		...
		this->enemy.setTexture(enemyTextures["ENEMY_TYPE_4"]);
		enemy.setTextureRect(sf::IntRect(0, 0, 64, 64));
		this->enemyHp = 80;
		break;
	default:
		this->enemy.setSize(sf::Vector2f(200.f, 200.f));
		this->enemy.setFillColor(sf::Color::Green);
		this->enemy.setTexture(enemyTextures["ENEMY_TYPE_5"]);
		enemy.setTextureRect(sf::IntRect(0, 0, 64, 64));
		this->enemyHp = 100;
		break;
	}
...
void Game::updateEnemies()
{
	...
	*/
	...
	for (int i = 0; i < this->enemies.size(); i++)
	{
		...
		if (direction.x < 0) {
			// Enemy is moving left, mirror the texture
			this->enemies[i].setScale(0.5f, 0.5f);
		}
		else {
			// Enemy is moving right, restore the original texture orientation
			this->enemies[i].setScale(-0.5f, 0.5f);
		}
...
void Game::restart() {
	...
	// Load the worldBackground texture from a PNG file
...
void Game::updateLevel() {
	// Update game elements based on points
	if (this->points < 200) {
		this->maxEnemies = 10;
		this->speed = 1.f;
		this->attackCooldownMax = 100000.f;
		this->target = 0;
	}
	...
	if (this->points >= 700) {
			...
			this->target = 2;
			this->maxEnemies = 50;
		if (isFirstMaxHPIncrease) {
			this->hpMax = 15;
			this->hp = this->hpMax;
			this->player.setScale(sf::Vector2f(2.f, 2.f));
			isFirstMaxHPIncrease = false; 
		}
	}

	if (this->points >= 700) {
		if (isFirstTextureChange) {
			if (!this->playerTexture.loadFromFile("Assets/player/house.png")) {
				std::cerr << "Failed to load player texture!" << std::endl;
			}
			if (!this->worldBackgroundTex.loadFromFile("Assets/BG/BAG.png")) {
				std::cerr << "Failed to load player texture!" << std::endl;
			}
			player.setTextureRect(sf::IntRect(0, 0, 64, 64));
			isFirstTextureChange = false; 
		}
	}
	...
	if (this->points >= 1100) {
			...
			this->attackCooldownMax = 8.f;
			this->target = 3;
		if (isSecondMaxHPIncrease) {
			this->hpMax = 20;
			this->hp = this->hpMax;
			isSecondMaxHPIncrease = false; 
		}
	}

	if (this->points >= 1100) {
		
		if (isSecondTextureChange) {
			if (!this->playerTexture.loadFromFile("Assets/player/house.png")) {
				std::cerr << "Failed to load player texture!" << std::endl;
			}
			if (!this->worldBackgroundTex.loadFromFile("Assets/BG/BAG Final.png")) {
				std::cerr << "Failed to load player texture!" << std::endl;
			}
			player.setTextureRect(sf::IntRect(0, 64 * 1, 64, 64));
			isSecondTextureChange = false; 
		}
	}
	...
	if (this->points >= 1500) {
			...
			this->attackCooldownMax = 8.f;
			this->target = 4;
		if (isThirdMaxHPIncrease) {
			this->hpMax = 25;
			this->hp = this->hpMax;
			isThirdMaxHPIncrease = false; 
		}
	}
	...
	if (this->points >= 2000) {
			...
			this->target = 5;
		if (isForthMaxHPIncrease) {
			this->hpMax = 30;
			this->hp = this->hpMax;
			isForthMaxHPIncrease = false; 
		}
	}
}
